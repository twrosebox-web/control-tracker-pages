...
var today=function(){return now().slice(0,10)};
...
function modal(title,sub,body,wide){
  ...
  box.innerHTML='<div class="modal-head"><div><h2>'+esc(title)+'</h2><p>'+esc(sub||'')+'</p></div><button class="btn icon" onclick="app.closeModal()">×</button></div>'+body;
...
function focusQuickHtml(){
  var timer=loadTimer(),remaining=timerRemaining(timer),openTasks=(d().tasks||[]).filter(function(t){return t.status!=='done'}).sort(function(a,b){return String(a.dueDate||'9999').localeCompare(String(b.dueDate||'9999'))}),linked=task(timer.taskId),label=timer.label||(linked&&linked.name)||'';
  return '<section class="ops-focus-quick" id="opsFocusQuick"><div class="ops-focus-quick-title"><span>🍅</span><span>快速專注</span></div><select id="homeFocusTask" onchange="financeApp.quickTargetChanged()"><option value="">選擇今天要推進的工作</option>'+openTasks.map(function(t){var p=project(t.projectId);return '<option value="'+esc(t.id)+'" '+(t.id===timer.taskId?'selected':'')+'>'+esc((p?p.name+'｜':'')+t.name)+'</option>'}).join('')+'</select><div class="ops-focus-quick-time" id="homeFocusClock">'+clock(remaining)+'</div><button class="btn primary" id="homeFocusToggle" onclick="financeApp.quickToggle()">'+(timer.running?'暫停':'開始')+'</button><button class="btn ops-focus-detail" onclick="financeApp.open(\'focus\')">詳細</button><div class="meta ops-focus-detail" id="homeFocusDetail">'+esc(label?(timer.running?'正在：'+label:'已選擇：'+label):'從專案工作直接開始，完成後會留下成果與下一步。')+'</div></section>';
}
...
function quickTargetChanged(){var timer=loadTimer(),selected=val('homeFocusTask'),linked=task(selected);if(timer.running&&selected!==timer.taskId)return toast('計時中請先暫停，再切換工作');timer.taskId=selected;timer.projectId=linked&&linked.projectId||'';timer.label='';saveTimer(timer);drawTimer()}
function quickToggle(){var timer=loadTimer(),selected=val('homeFocusTask')||timer.taskId,linked=task(selected),label=timer.label||(linked&&linked.name)||'';if(!label)return alert('請先選擇一項工作。');timer.taskId=selected;timer.projectId=linked&&linked.projectId||timer.projectId||'';if(timer.running){timer.elapsedSeconds=timerElapsed(timer);timer.running=false;timer.startedAt=null}else{if(timerRemaining(timer)<=0)timer.elapsedSeconds=0;timer.running=true;timer.startedAt=Date.now()}saveTimer(timer);drawTimer();startTick()}
function startTask(taskId){var timer=loadTimer(),linked=task(taskId);if(!linked)return;if(timer.running&&timer.taskId!==taskId&&!confirm('目前正在計時，確定切換到「'+linked.name+'」？'))return;timer.taskId=taskId;timer.projectId=linked.projectId||'';timer.label='';timer.elapsedSeconds=0;timer.running=true;timer.startedAt=Date.now();timer.interruptions=0;timer.note='';saveTimer(timer);app.closeModal();open('focus');toast('已開始專注：'+linked.name)}
...
function drawTimer(){var timer=loadTimer(),remaining=timerRemaining(timer),clockEl=document.getElementById('focusClock'),homeClock=document.getElementById('homeFocusClock'),progress=document.getElementById('focusProgress'),label=document.getElementById('focusLabel'),homeDetail=document.getElementById('homeFocusDetail'),homeToggle=document.getElementById('homeFocusToggle'),linked=task(timer.taskId),name=timer.label||(linked&&linked.name)||'';if(clockEl)clockEl.textContent=clock(remaining);if(homeClock)homeClock.textContent=clock(remaining);if(progress)progress.style.width=Math.min(100,Math.max(0,(1-remaining/Math.max(1,timer.totalSeconds))*100))+'%';if(label)label.textContent=name||'尚未選擇工作';if(homeDetail)homeDetail.textContent=name?(timer.running?'正在：'+name:'已選擇：'+name):'從專案工作直接開始，完成後會留下成果與下一步。';if(homeToggle)homeToggle.textContent=timer.running?'暫停':'開始';if(remaining<=0&&timer.running)finishFocus(false)}
...
function enhanceDashboard(){var title=document.getElementById('pageTitle'),content=document.getElementById('content');if(!title||!content||title.textContent.indexOf('首頁')<0)return;var todayBoard=document.getElementById('v6TodayBoard'),quick=document.getElementById('opsFocusQuick');if(!quick){if(todayBoard)todayBoard.insertAdjacentHTML('beforebegin',focusQuickHtml());else content.insertAdjacentHTML('afterbegin',focusQuickHtml())}var alerts=document.getElementById('opsFinanceAlerts');if(!alerts){todayBoard=document.getElementById('v6TodayBoard');if(todayBoard)todayBoard.insertAdjacentHTML('afterend',alertsHtml());else document.getElementById('opsFocusQuick').insertAdjacentHTML('afterend',alertsHtml())}drawTimer()}

...
function enhanceDashboard(){var title=document.getElementById('pageTitle'),content=document.getElementById('content');if(!title||!content||title.textContent.indexOf('首頁')<0)return;var todayBoard=document.getElementById('v6TodayBoard'),quickGrid=document.getElementById('opsDashboardQuick');if(!quickGrid){var quickHtml='<div class="ops-dashboard-quick-grid" id="opsDashboardQuick">'+focusQuickHtml()+quickFinanceHtml()+'</div>';if(todayBoard)todayBoard.insertAdjacentHTML('beforebegin',quickHtml);else content.insertAdjacentHTML('afterbegin',quickHtml)}var alerts=document.getElementById('opsFinanceAlerts');if(!alerts){todayBoard=document.getElementById('v6TodayBoard');if(todayBoard)todayBoard.insertAdjacentHTML('afterend',alertsHtml());else document.getElementById('opsDashboardQuick').insertAdjacentHTML('afterend',alertsHtml())}drawTimer()}
